...
export const useMockAI = () => {
  ...
  const generateChartFromPrompt = async (prompt: string): Promise<Options> => {
    setIsLoading(true);
    ...
    try {
      ...
      if (chartType === 'pie') {
        ...
        const dataSeries1 = categories.map(
          () => Math.floor(Math.random() * 100) + 20
        );
        const dataSeries2 = categories.map(
          () => Math.floor(Math.random() * 100) + 20
        );
